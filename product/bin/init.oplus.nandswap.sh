...
function configure_zram_parameters() {
	...
	if [ -f /sys/block/zram0/disksize ]; then
		...
		if [ $MemTotal -le 524288 ]; then
			...
			#config 4GB zramsize with ramsize over 8GB
			swap_size_mb=4096
		fi

		zram_increase=$(expr $nandswap_size \/ 1024 \/ 1024)
		if [ $zram_increase -gt 3072 ]; then
			zram_increase=3072
		fi
...
function configure_swappiness()
{
	...
	if [ "$fn_enable" == "true" -a "$nandswap_size" != "0" -a $hybridswap_has_insmod -eq 1 ]; then
		echo "direct_swappiness=60" > /proc/${oplus_path}/swappiness_para
	else
		echo "direct_swappiness=60" > /proc/${oplus_path}/swappiness_para
	fi
}
...
function check_fn_config(){
	...
	if [ ! -n "$is_ufs" ]; then
		...
		if [[ -n "$prjname" && "$prjname" == "21336" ]]; then
			emmc_device=1
		else
...
function nandswap_init(){
	...
	if [ "$set_dio" == "success" ]; then
		if [ $hybridswap_has_insmod -eq 1 ]; then
			...
			echo "3 0 99 0 0 0 100 399 60 $zram2ufs_ratio 0 400 499 50 0 0 " > /dev/memcg/memory.swapd_memcgs_param
			...
			MemTotal=${MemTotalStr:16:8}
			if [ $MemTotal -le 4194304 ]; then
				echo "2000 1600 2000 1536" > /dev/memcg/memory.avail_buffers
			elif [ $MemTotal -le 6291456 ]; then
				echo "2000 1600 2000 1536" > /dev/memcg/memory.avail_buffers
			elif [ $MemTotal -le 8388608 ]; then
				echo "2200 1800 2200 1736" > /dev/memcg/memory.avail_buffers
			else
				echo "2000 1600 2000 1536" > /dev/memcg/memory.avail_buffers
			fi
